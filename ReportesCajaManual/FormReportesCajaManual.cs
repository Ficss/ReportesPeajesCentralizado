...
{
    public partial class FormReportesCajaManual : KryptonForm
    {
        #region Singleton
        private static FormReportesCajaManual frm = null;
        public static FormReportesCajaManual Instance()
        {

            if (frm == null)
            {
                frm = new FormReportesCajaManual();
            }
            return frm;
        }
        #endregion
        public FormReportesCajaManual()
        {
            InitializeComponent();
        ...
        private void FormReportesCajaManual_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(Properties.Settings.Default.CM);
        ...
        private void btnDetalleDiario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string con = Properties.Settings.Default.CM;
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos", connection))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        connection.Close();
                        this.sp_informe_total_vehiculosTableAdapter.Fill(this.CajaManualDataSet.sp_informe_total_vehiculos, Convert.ToDateTime(fecha));
                    }
                }
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos_rango", connection))
                    {
                        ...
                        this.sp_informe_total_vehiculos_rangoTableAdapter.Fill(this.CajaManualDataSet.sp_informe_total_vehiculos_rango, Convert.ToDateTime(fecha_i), Convert.ToDateTime(fecha_f));
                    }
                }
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos_rango", connection))
                    {
                        ...
                        this.sp_informe_total_vehiculos_rangoTableAdapter.Fill(this.CajaManualDataSet.sp_informe_total_vehiculos_rango, Convert.ToDateTime(firstDayOfMonth), Convert.ToDateTime(lastDayOfMonth));
                    }
                }
