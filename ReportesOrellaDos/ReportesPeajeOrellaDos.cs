...
{
    public partial class ReportesPeajeOrellaDos : KryptonForm
    {
        #region Singleton
        private static ReportesPeajeOrellaDos frm = null;
        public static ReportesPeajeOrellaDos Instance()
        {
            if (frm == null)
            {
                frm = new ReportesPeajeOrellaDos();
            }
            return frm;
        }
        #endregion
        #region Constructor por defecto
        public ReportesPeajeOrellaDos()
        {
            InitializeComponent();
        ...
        #endregion
        #region Load
        private void ReportesPeajeOrellaDos_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(Properties.Settings.Default.OD);
            if (Prueba.QuickOpen(con, 1000) == false)
            {
                MessageBox.Show("No se puso establecer una conexión a la base de datos.\n  " +
                                ...
                                " -Peaje está cerrado.\n" +
                                " -El cable de red está desconectado de su computador", "Peaje Orella Dos");
        ...
        private void btnBuscarDiario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string con = Properties.Settings.Default.OD;
