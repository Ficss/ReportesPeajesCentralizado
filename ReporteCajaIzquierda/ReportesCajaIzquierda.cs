...
{
    public partial class ReportesCajaIzquierda : KryptonForm
    {
        #region Singleton
        private static ReportesCajaIzquierda frm = null;
        public static ReportesCajaIzquierda Instance()
        {

            if (frm == null)
            {
                frm = new ReportesCajaIzquierda();
            }
            return frm;
        }
        #endregion
        #region Constructor por defecto
        public ReportesCajaIzquierda()
        {
            InitializeComponent();
        ...
        #endregion
        #region Load
        private void ReportesCajaIzquierda_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(Properties.Settings.Default.RI);
            if (Prueba.QuickOpen(con, 600) == false)
            {
                MessageBox.Show("No se puso establecer una conexión a la base de datos.\n  " +
                                ...
                                " -Peaje está cerrado.\n" +
                                " -El cable de red está desconectado de su computador");
        ...
        private void btnDetalleDiario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string con = Properties.Settings.Default.RI;
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos", connection))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        connection.Close();
                        this.sp_informe_total_vehiculosTableAdapter.Fill(this.CajaIzquierdaDataSet.sp_informe_total_vehiculos, Convert.ToDateTime(fecha));
                    }
                }
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos_rango", connection))
                    {
                        ...
                        this.sp_informe_total_vehiculos_rangoTableAdapter.Fill(this.CajaIzquierdaDataSet.sp_informe_total_vehiculos_rango, Convert.ToDateTime(fecha_i), Convert.ToDateTime(fecha_f));
                    }
                }
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(con))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_informe_total_vehiculos_rango", connection))
                    {
                        ...
                        this.sp_informe_total_vehiculos_rangoTableAdapter.Fill(this.CajaIzquierdaDataSet.sp_informe_total_vehiculos_rango, Convert.ToDateTime(firstDayOfMonth), Convert.ToDateTime(lastDayOfMonth));
                    }
                }
