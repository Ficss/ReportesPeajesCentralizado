...
namespace ReportesBriceno
{
    ...
    {
        #region Singleton
        private static ReportesPeajeBriceno frm = null;
        public static ReportesPeajeBriceno Instance()
        {

            if (frm == null)
            {
                frm = new ReportesPeajeBriceno();
            }
            return frm;
        }
        #endregion
        #region Constructor por defecto
        public ReportesPeajeBriceno()
        {
            InitializeComponent();
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(con))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("sp_informe_diario_manual", connection))
                    {
                        ...
                        this.informe_diario_manualTableAdapter.Fill(this.DataSet1.informe_diario_manual);
                    }
                }
                ReportParameter[] rparams = new ReportParameter[] {
                new ReportParameter("fecha", fecha)
